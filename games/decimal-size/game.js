/* games/decimal-size/game.js */

'use strict';

// ── Constants ────────────────────────────────────────────────
const TOTAL_QUESTIONS  = 10;
const TIMEOUT_MS       = 5000;   // 5 seconds per question
...
// ── Helpers (also used by the question generator) ────────────
...
// ── Question generator ───────────────────────────────────────
// "가장 큰(작은) 소수는?" — 서로 다른 소수 4개 중 가장 큰(또는 작은) 것 하나가 정답.
// 값이 모두 달라 극값이 유일 → 정답이 하나로 확정됨. 소수 자릿수를 섞어
// 0.7 과 0.65 처럼 자릿수가 다른 소수의 크기 비교를 익히게 한다.
function genDecimalQuestion() {
  ...
  while (arr.length < 4 && guard < 200) {
    guard++;
    let v;
    if (useHundredths && Math.random() < 0.6) v = rand(11, 99) / 100;  // 0.11 ~ 0.99
    else v = rand(1, 9) / 10;                                          // 0.1 ~ 0.9
    const key = v.toFixed(2);
    if (vals.has(key)) continue;
    vals.add(key);
    arr.push(v);
  }
  if (arr.length < 4) return genDecimalQuestion();

  const askMax = Math.random() < 0.5;
  const target = askMax ? Math.max(...arr) : Math.min(...arr);
  const a = String(target);
  const o = arr.filter(x => x !== target).map(String);
  if (o.length !== 3) return genDecimalQuestion();

  return { q: askMax ? '가장 큰 소수는?' : '가장 작은 소수는?', a: a, o: o };
}
...
function buildPool() {
  ...
  const seen = new Set();
  let guard = 0;
  while (pool.length < 40 && guard < 3000) {
    guard++;
    const item = genDecimalQuestion();
    const key = item.q + '|' + item.a + '|' + item.o.join(',');
...
let QUESTION_POOL = buildPool();  // 30개 이상 (자가 게이트 a)
