...
// ── Direction library (4 cardinal directions) ────────────────
// angle는 arrowSVG의 위쪽(↑) 기준 시계방향 회전각.
const DIRECTIONS = [
  { key: 'up',    name: '위',    angle: 0,    cls: 'dir-up'    },
  { key: 'left',  name: '왼쪽',  angle: -90,  cls: 'dir-left'  },
  { key: 'right', name: '오른쪽', angle: 90,   cls: 'dir-right' },
  { key: 'down',  name: '아래',  angle: 180,  cls: 'dir-down'  },
];

function dirByKey(key) {
  return DIRECTIONS.find(d => d.key === key);
}

// 큰 화살표 SVG (상단 디스플레이 — 따라야 할 대상)
function arrowDisplaySVG(angle) {
  return `<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
    <g transform="rotate(${angle} 50 50)">
      <rect x="40" y="24" width="20" height="48" fill="#2C2C2C"/>
      <polygon points="18,32 82,32 50,6" fill="#2C2C2C"/>
    </g>
  </svg>`;
}
...
function arrowBtnSVG(angle, bgColor) {
  return `<svg viewBox="0 0 110 78" xmlns="http://www.w3.org/2000/svg">
    <rect x="2" y="2" width="106" height="74" rx="14" ry="14"
          fill="${bgColor}" opacity="0.95" stroke="#2C2C2C" stroke-width="3"/>
    <g transform="rotate(${angle} 55 39)">
      <rect x="49" y="26" width="12" height="26" fill="#fff"/>
      <polygon points="37,30 73,30 55,13" fill="#fff"/>
    </g>
  </svg>`;
}
...
let scores        = [];
let roundLog      = [];
let currentDir    = null;   // 정답 (화살표가 가리키는 방향)
let currentWord   = null;   // 함정 낱말
let dqSet         = new Set();
let phase         = 'idle';
...
const trickWord     = document.getElementById('trickWord');
...
// 라운드 풀 생성: 화살표 방향(정답) + 함정 낱말.
// 약 70%는 낱말≠화살표(불일치), 30%는 낱말=화살표(일치)로 섞어 함정을 예측 못하게 한다.
function buildRoundPool() {
  const pool = [];
  DIRECTIONS.forEach(arrow => {
    // 불일치 3종 (다른 방향 낱말)
    DIRECTIONS.filter(w => w.key !== arrow.key).forEach(word => {
      pool.push({ arrowKey: arrow.key, wordKey: word.key });
      pool.push({ arrowKey: arrow.key, wordKey: word.key });
    });
    // 일치 1종
    pool.push({ arrowKey: arrow.key, wordKey: arrow.key });
  });
  return pool; // 4 * (3*2 + 1) = 28개
}
...
function renderIntroArrows() {
  ...
  const sample = [
    { angle: -90, color: '#B3E5FC', stroke: '#0288D1' },  // 왼쪽 화살표
    { angle: 0,   color: '#FFCDD2', stroke: '#E53935' },  // 위 화살표
    { angle: 90,  color: '#C8E6C9', stroke: '#388E3C' },  // 오른쪽 화살표
  ];
  sample.forEach(s => {
    ...
    wrap.innerHTML = `<svg viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">
      ...
      <g transform="rotate(${s.angle} 40 40)">
        <rect x="33" y="24" width="14" height="30" fill="#2C2C2C"/>
        <polygon points="16,30 64,30 40,8" fill="#2C2C2C"/>
      </g>
    </svg>`;
...
// ── Populate answer buttons (고정 십자 배치의 방향 4개) ──────────
function populateAnswerBtns() {
  for (let i = 0; i < playerCount; i++) {
    ...
    DIRECTIONS.forEach(dir => {
      ...
      btn.className = `answer-btn ${dir.cls}`;
      btn.dataset.player = i;
      btn.dataset.choice = dir.key;
      btn.setAttribute('aria-label', `P${i + 1} ${dir.name}`);
      btn.innerHTML = arrowBtnSVG(dir.angle, PLAYER_CONFIG[i].dot);

      onTap(btn, () => handleAnswerTap(i, dir.key, btn));
...
function resetBtnsForRound() {
  for (let i = 0; i < playerCount; i++) {
    ...
    btns.forEach(btn => {
      btn.className = `answer-btn ${dirByKey(btn.dataset.choice).cls}`;
      btn.disabled = false;
      if (dqSet.has(i)) {
...
function handleAnswerTap(playerIdx, chosenKey, btn) {
  ...
  const correct = (chosenKey === currentDir.key);
...
function resolveRound(winnerIdx) {
  ...
  getAnswerBtns(winnerIdx).forEach(btn => {
    if (btn.dataset.choice === currentDir.key) {
...
function handleTimeout() {
  ...
  problemStatus.textContent = `시간 초과! 정답: ${currentDir.name}`;
...
function loadRound() {
  phase        = 'active';
  const spec   = gameRounds[roundIdx];
  currentDir   = dirByKey(spec.arrowKey);
  currentWord  = dirByKey(spec.wordKey);
  dqSet        = new Set();

  questionCounter.textContent = `${roundIdx + 1} / ${TOTAL_ROUNDS}`;
  flagDisplay.innerHTML = arrowDisplaySVG(currentDir.angle);
  trickWord.textContent = currentWord.name;
  trickWord.classList.toggle('is-match', currentDir.key === currentWord.key);
...
// 같은 화살표 방향이 연속되지 않게 라운드 목록을 뽑는다.
function pickGameRounds() {
  const shuffled = shuffle(ROUND_POOL);
  const picked = [];
  for (const r of shuffled) {
    if (picked.length >= TOTAL_ROUNDS) break;
    const prev = picked[picked.length - 1];
    if (prev && prev.arrowKey === r.arrowKey) continue;
    picked.push(r);
  }
  // 부족하면 남은 것으로 채운다 (연속 제약 완화)
...
function showResult() {
  ...
  roundLog.forEach((log, idx) => {
    ...
    let cells = `<td style="text-align:left;font-size:0.82rem;">${idx + 1}. 화살표 ${log.dirName}</td>`;
