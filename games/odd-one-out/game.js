/* games/odd-one-out/game.js — 패턴 A (동시 반응형), 골든 템플릿: flag-quiz */
...
// ── Word Sets ────────────────────────────────────────────────
// 각 세트: 같은 무리 낱말 3개 + 어울리지 않는 1개(odd).
// words 배열의 4개는 라운드마다 섞여 보기로 표시된다.
const ALL_SETS = [
  { cat: '과일',   words: ['사과', '포도', '딸기'],     odd: '당근' },
  { cat: '동물',   words: ['강아지', '고양이', '토끼'], odd: '책상' },
  { cat: '탈것',   words: ['자동차', '버스', '비행기'], odd: '바나나' },
  { cat: '채소',   words: ['당근', '오이', '양파'],     odd: '호랑이' },
  { cat: '색깔',   words: ['빨강', '파랑', '노랑'],     odd: '사자' },
  { cat: '운동',   words: ['축구', '농구', '야구'],     odd: '연필' },
  { cat: '곤충',   words: ['나비', '개미', '벌'],       odd: '자전거' },
  { cat: '직업',   words: ['의사', '경찰', '소방관'],   odd: '딸기' },
  { cat: '가구',   words: ['침대', '의자', '책상'],     odd: '고양이' },
  { cat: '학용품', words: ['연필', '지우개', '가위'],   odd: '코끼리' },
  { cat: '날씨',   words: ['비', '눈', '바람'],         odd: '책' },
  { cat: '신체',   words: ['손', '발', '코'],           odd: '버스' },
  { cat: '악기',   words: ['피아노', '기타', '북'],     odd: '토마토' },
  { cat: '꽃',     words: ['장미', '튤립', '백합'],     odd: '개구리' },
  { cat: '음식',   words: ['밥', '국', '빵'],           odd: '구름' },
  { cat: '바다생물', words: ['문어', '상어', '게'],     odd: '사과' },
  { cat: '새',     words: ['참새', '까치', '비둘기'],   odd: '책상' },
  { cat: '옷',     words: ['바지', '치마', '양말'],     odd: '컵' },
  { cat: '주방도구', words: ['냄비', '그릇', '숟가락'], odd: '비행기' },
  { cat: '도형',   words: ['동그라미', '세모', '네모'], odd: '강아지' },
  { cat: '과일',   words: ['수박', '참외', '복숭아'],   odd: '가방' },
  { cat: '동물',   words: ['코끼리', '기린', '원숭이'], odd: '모자' },
  { cat: '탈것',   words: ['기차', '배', '오토바이'],   odd: '포도' },
  { cat: '채소',   words: ['배추', '무', '시금치'],     odd: '나비' },
  { cat: '운동',   words: ['수영', '태권도', '줄넘기'], odd: '의자' },
  { cat: '곤충',   words: ['잠자리', '메뚜기', '무당벌레'], odd: '우유' },
  { cat: '직업',   words: ['선생님', '요리사', '가수'], odd: '당근' },
  { cat: '색깔',   words: ['초록', '보라', '분홍'],     odd: '토끼' },
  { cat: '신체',   words: ['눈', '입', '귀'],           odd: '트럭' },
  { cat: '악기',   words: ['바이올린', '드럼', '플루트'], odd: '딸기' },
  { cat: '꽃',     words: ['무궁화', '진달래', '개나리'], odd: '고래' },
  { cat: '음식',   words: ['김치', '라면', '떡'],       odd: '지우개' },
];
...
let roundIdx      = 0;
let scores        = [];
let roundLog      = [];    // { answer, winnerIdx(-1=timeout), dqPlayers[], timedOut }
let currentItem   = null;  // { cat, words, odd }
let currentChoices = [];   // 4 words for this round (shuffled, includes odd)
let dqSet         = new Set();
let phase         = 'idle';
let timerHandle   = null;
let nextHandle    = null;
let timeRemaining = ROUND_TIME;
let gameRounds    = [];    // 10 selected sets
...
// ── Prompt illustration (shown in .flag-display each round) ──
const PROMPT_SVG = `<svg viewBox="0 0 120 80" xmlns="http://www.w3.org/2000/svg">
  <rect width="120" height="80" fill="#fff"/>
  <circle cx="50" cy="34" r="18" fill="none" stroke="#7E57C2" stroke-width="6"/>
  <line x1="63" y1="47" x2="78" y2="62" stroke="#7E57C2" stroke-width="7" stroke-linecap="round"/>
  <text x="50" y="40" text-anchor="middle" font-size="18" font-weight="900" fill="#7E57C2" font-family="sans-serif">?</text>
  <text x="60" y="75" text-anchor="middle" font-size="12" font-weight="900" fill="#2C2C2C" font-family="sans-serif">다른 하나는?</text>
</svg>`;
...
// 보기 4개 만들기: 같은 무리 3개 + odd, 섞어서 반환
function makeChoices(item) {
  return shuffle([...item.words, item.odd]);
}
...
function handleAnswerTap(playerIdx, chosenName, btn) {
  ...
  const correct = (chosenName === currentItem.odd);
...
function resolveRound(winnerIdx) {
  ...
  getAnswerBtns(winnerIdx).forEach(btn => {
    if (btn.dataset.choice === currentItem.odd) {
  ...
  problemStatus.textContent = `${winnerLabel} 정답! (${currentItem.cat} 무리)`;

  roundLog.push({
    answer: currentItem.odd,
    winnerIdx,
    dqPlayers: [...dqSet],
    timedOut: false,
  });
...
function handleTimeout() {
  ...
  problemStatus.textContent = `시간 초과! 정답: ${currentItem.odd}`;
...
function loadRound() {
  ...
  flagDisplay.innerHTML = PROMPT_SVG;
...
// ── Start game ───────────────────────────────────────────────
function startGame() {
  gameRounds  = shuffle(ALL_SETS).slice(0, TOTAL_ROUNDS);
  roundIdx    = 0;
...
function showResult() {
  ...
  roundLog.forEach((log, idx) => {
    ...
    let cells = `<td style="text-align:left;font-size:0.82rem;">${idx + 1}. ${log.answer}</td>`;
