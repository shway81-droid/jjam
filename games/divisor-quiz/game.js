...
// ── Question generator ───────────────────────────────────────
// "N의 약수가 아닌 수는?" — 정답은 N의 약수가 아닌 수 1개,
// 오답 3개는 모두 실제 N의 약수 → 정답이 하나로 확정됨.
// 약수가 풍부한 합성수만 사용해 보기 3개(약수)를 확보한다.
const DIVISOR_NS = [
  12, 14, 15, 16, 18, 20, 21, 24, 25, 27, 28, 30, 32, 33, 35, 36,
  40, 42, 44, 45, 48, 50, 54, 56, 60, 63, 64, 70, 72,
];

function divisorsOf(n) {
  // 1과 n을 제외한 진약수 중심 + n 포함(1은 너무 뻔해 제외)
  const d = [];
  for (let i = 2; i <= n; i++) if (n % i === 0) d.push(i);
  return d; // 항상 n 포함
}

function genDivisorQuestion() {
  const n = DIVISOR_NS[rand(0, DIVISOR_NS.length - 1)];
  const divs = divisorsOf(n);
  if (divs.length < 3) return genDivisorQuestion();

  const three = shuffle(divs).slice(0, 3);

  // n 이하에서 n의 약수가 아니고 보기와 겹치지 않는 수 하나 고르기
  let nd, guard = 0;
  do {
    nd = rand(2, n);
    guard++;
  } while ((n % nd === 0 || three.indexOf(nd) !== -1) && guard < 300);

  if (n % nd === 0) return genDivisorQuestion();

  return { q: `${n}의 약수가 아닌 수는?`, a: String(nd), o: three.map(String) };
}
