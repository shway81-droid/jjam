...
// 운동 종목 데이터 — 종목 이름은 모두 고유(보기 모호성 방지).
const SPORTS_DATA = [
  { emoji: '⚽', name: '축구' },
  { emoji: '🏀', name: '농구' },
  { emoji: '⚾', name: '야구' },
...
let scores       = [];
let questionLog  = [];
let currentQuestion = null;      // { emoji, answer, choices }
let dqSet        = new Set();
let phase        = 'idle';
...
function generateQuestion() {
  const item = SPORTS_DATA[rand(0, SPORTS_DATA.length - 1)];

  // 3 distinct wrong names from other entries (names are globally unique).
  const others = SPORTS_DATA.filter(d => d.name !== item.name);
  shuffle(others);
  const wrong = others.slice(0, 3).map(d => d.name);

  const choices = shuffle([item.name, ...wrong]);

  return { emoji: item.emoji, answer: item.name, choices };
}
...
function nextQuestion() {
  ...
  problemExpr.innerHTML =
    `<span class="sport-emoji">${currentQuestion.emoji}</span>`;
...
function showResult() {
  ...
  resultTableBody.innerHTML = questionLog.map((q, ri) => {
    ...
    return `<tr>
      <td>${ri + 1}</td>
      <td style="font-size:1.3rem">${q.emoji}</td>
