...
function generateQuestion() {
  const { lenMin, lenMax } = pathRange();
  let path = buildPath(lenMin, lenMax);
  if (!path) path = buildPath(3, 5); // safety fallback (very rare)

  const floor = new Set(path);
  const start = path[0];
  const end   = path[path.length - 1];
  const answer = path.length - 1; // number of moves

  // Wrong choices: numbers near the answer, distinct, >= 1
  const pool = [];
  ...
  for (let i = 0; i < GRID_N * GRID_N; i++) {
    if (i === start)        cells += `<span class="maze-cell start">🐭</span>`;
    else if (i === end)     cells += `<span class="maze-cell end">🧀</span>`;
    else if (floor.has(i))  cells += `<span class="maze-cell floor"></span>`;
    else                    cells += `<span class="maze-cell wall"></span>`;
  }
  const html = `<div class="maze-grid">${cells}</div>`;
...
function showResult() {
  ...
  resultTableBody.innerHTML = questionLog.map((q, ri) => {
    ...
    return `<tr>
      <td>${ri + 1}</td>
      <td style="font-weight:800">${q.answer}칸</td>
      ${cells}
    </tr>`;
  }).join('');
