/* games/shape-name/game.js */

'use strict';

// ── Constants ────────────────────────────────────────────────
const TOTAL_QUESTIONS  = 10;
const TIMEOUT_MS       = 6000;   // 6 seconds per question (shape naming)
...
// ── Question pool ────────────────────────────────────────────
// 도형 그림(SVG)을 보고 이름을 고른다. 도형은 기본형을 명확한 방향으로 그려
// 정답이 하나로 확정되게 한다(마름모는 정사각형과 헷갈리지 않게 대각선 길이를 다르게).
// `q`는 SVG(HTML)이므로 innerHTML로 렌더한다.
const F = 'fill="#7E57C2" stroke="#2C2C2C" stroke-width="3"';
const card = (shape) =>
  '<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" class="shape-svg">' +
  '<rect x="2" y="2" width="96" height="96" rx="10" fill="#FFFDF7" stroke="#2C2C2C" stroke-width="2.5"/>' +
  shape + '</svg>';
const QUESTION_POOL = [
  { q: card(`<circle cx="50" cy="50" r="36" ${F}/>`), a: '원', o: ['타원', '정사각형', '삼각형'] },
  { q: card(`<polygon points="50,14 86,82 14,82" ${F}/>`), a: '삼각형', o: ['사각형', '오각형', '원'] },
  { q: card(`<rect x="18" y="18" width="64" height="64" ${F}/>`), a: '정사각형', o: ['직사각형', '마름모', '원'] },
  { q: card(`<rect x="12" y="30" width="76" height="40" ${F}/>`), a: '직사각형', o: ['정사각형', '평행사변형', '사다리꼴'] },
  { q: card(`<polygon points="50,12 88,40 74,84 26,84 12,40" ${F}/>`), a: '오각형', o: ['육각형', '삼각형', '사각형'] },
  { q: card(`<polygon points="50,10 85,30 85,70 50,90 15,70 15,30" ${F}/>`), a: '육각형', o: ['오각형', '삼각형', '원'] },
  { q: card(`<polygon points="50,8 78,50 50,92 22,50" ${F}/>`), a: '마름모', o: ['정사각형', '직사각형', '삼각형'] },
  { q: card(`<polygon points="30,25 70,25 88,78 12,78" ${F}/>`), a: '사다리꼴', o: ['평행사변형', '직사각형', '마름모'] },
  { q: card(`<polygon points="28,26 92,26 72,78 8,78" ${F}/>`), a: '평행사변형', o: ['직사각형', '사다리꼴', '마름모'] },
  { q: card(`<ellipse cx="50" cy="50" rx="42" ry="26" ${F}/>`), a: '타원', o: ['원', '직사각형', '반원'] },
  { q: card(`<path d="M12,64 A38,38 0 0 1 88,64 Z" ${F}/>`), a: '반원', o: ['원', '부채꼴', '삼각형'] },
  { q: card(`<polygon points="50,10 61,38 91,38 67,57 76,86 50,68 24,86 33,57 9,38 39,38" ${F}/>`), a: '별', o: ['육각형', '오각형', '원'] },
  { q: card(`<path d="M50,50 L50,12 A38,38 0 0 1 88,50 Z" ${F}/>`), a: '부채꼴', o: ['반원', '삼각형', '원'] },
];
