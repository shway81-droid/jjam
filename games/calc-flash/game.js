...
(function () {
  ...
  // ===== 계산 · 목표 =====
  var TARGETS = [5, 6, 7, 8, 9, 10, 11, 12];
  ...
  // Build a small arithmetic expression. If wantValue is a number, produce one that
  // evaluates to exactly that value; otherwise a random distractor (value != avoid).
  function makeExpr(wantValue, avoid) {
    if (typeof wantValue === 'number') {
      var t = wantValue;
      if (Math.random() < 0.6) {                       // addition: a + b = t
        var a = rint(1, t - 1);
        return { text: a + '+' + (t - a), value: t };
      }
      var c = t + rint(1, 9);                          // subtraction: c - d = t
      return { text: c + '-' + (c - t), value: t };
    }
    // Distractor: random +,-,× within range, not equal to avoid.
    for (var g = 0; g < 40; g++) {
      var r = Math.random(), x, y, val, txt;
      if (r < 0.45)      { x = rint(1, 12); y = rint(1, 9);  val = x + y; txt = x + '+' + y; }
      else if (r < 0.8)  { x = rint(2, 18); y = rint(1, x);  val = x - y; txt = x + '-' + y; }
      else               { x = rint(2, 5);  y = rint(2, 4);  val = x * y; txt = x + '×' + y; }
      if (val !== avoid && val >= 1) return { text: txt, value: val };
    }
    return { text: '9+9', value: 18 };
  }
  ...
  function spawnBalloon(playerIdx) {
    ...
    // Expression on the balloon. ~40% of balloons are made to match the current
    // target so there is always something correct to hit; the rest are distractors.
    var expr = (Math.random() < 0.4)
      ? makeExpr(currentTarget)
      : makeExpr(undefined, currentTarget);
    ...
    // Tap to pop — only correct if the expression value equals the current target.
    var popped = false;
    function handlePop(e) {
      ...
      var isCorrect = (expr.value === currentTarget);
      if (isCorrect) {
  ...
  // ===== 목표 수 =====
  var targetValEl = document.getElementById('targetVal');
  function setTarget(n) {
    currentTarget = n;
    if (targetValEl) targetValEl.textContent = n;
    var chip = document.getElementById('hudTarget');
    if (chip) {
  ...
  function startGame() {
    ...
    targetTimer = setInterval(rotateTarget, 4200);
