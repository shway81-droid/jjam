/* games/group-count/game.js — 묶어 세기 (똑같은 묶음 → 곱셈 총개수 2~4인) */
...
// 라운드별 난이도 (묶음 수 K · 묶음당 개수 M)
// phase 1: 2~3묶음, 2~5개씩, 6초
// phase 2: 2~4묶음, 2~5개씩, 5초
// phase 3: 3~5묶음, 2~6개씩, 5초
const MS_ROUND_PLAN = [
  { phase: 1, timeLimit: 6 },
  { phase: 1, timeLimit: 6 },
  { phase: 2, timeLimit: 5 },
...
// 문항 생성
// K묶음 × M개 = 총 K*M개. 보기는 숫자 4개(정답 1 + 오답 3, 모두 다름).
// returns { groups:K, per:M, emoji, options:[String...], answer:String }
function msGenerateItem(phaseNum) {
  ...
  var answer = K * M;
  var emoji  = MS_EMOJIS[msRandInt(MS_EMOJIS.length)];

  // 그럴듯한 오답 후보들 (덧셈 혼동·인접 곱 등)
  var pool = [
    K + M, (K + 1) * M, (K - 1) * M, K * (M + 1), K * (M - 1),
    answer + M, answer - M, answer + 1, answer - 1, answer + K, answer - K,
  ];
  var opts = [answer];
  msShuffle(pool).forEach(function(v) {
  ...
  var pad = answer + 2;
  while (opts.length < 4) {
    if (opts.indexOf(pad) === -1) opts.push(pad);
    pad++;
  }
...
(function() {
  ...
  if (el) {
    el.innerHTML = '<svg viewBox="0 0 220 110" xmlns="http://www.w3.org/2000/svg">' +
      ...
      '<rect x="18" y="20" width="56" height="48" rx="8" fill="#FFFFFF" stroke="#2C2C2C" stroke-width="2.5"/>' +
      '<text x="30" y="40" font-size="16">🍎</text><text x="48" y="40" font-size="16">🍎</text>' +
      '<text x="30" y="60" font-size="16">🍎</text><text x="48" y="60" font-size="16">🍎</text>' +
      '<rect x="82" y="20" width="56" height="48" rx="8" fill="#FFFFFF" stroke="#2C2C2C" stroke-width="2.5"/>' +
      '<text x="94" y="40" font-size="16">🍎</text><text x="112" y="40" font-size="16">🍎</text>' +
      '<text x="94" y="60" font-size="16">🍎</text><text x="112" y="60" font-size="16">🍎</text>' +
      '<rect x="146" y="20" width="56" height="48" rx="8" fill="#FFFFFF" stroke="#2C2C2C" stroke-width="2.5"/>' +
      '<text x="158" y="40" font-size="16">🍎</text><text x="176" y="40" font-size="16">🍎</text>' +
      '<text x="158" y="60" font-size="16">🍎</text><text x="176" y="60" font-size="16">🍎</text>' +
      '<text x="110" y="98" text-anchor="middle" font-size="14" font-weight="900" fill="#26A69A">4씩 3묶음 = 12!</text>' +
      '</svg>';
  }
})();
...
function msHandleItemTimeout() {
  ...
  msProblemStatus.textContent = '⏰ 정답: ' + currentItem.per + '×' + currentItem.groups + ' = ' + currentItem.answer + '개';
...
function msLoadItem() {
  ...
  msProblemStatus.textContent = '모두 몇 개? (' + currentItem.per + '씩 ' + currentItem.groups + '묶음)';
  msProblemTimer.classList.remove('urgent');

  // 묶음 렌더 (K묶음 × M개)
  msScatter.innerHTML = '';
  for (var g = 0; g < currentItem.groups; g++) {
    var box = document.createElement('div');
    box.className = 'gc-group';
    for (var m = 0; m < currentItem.per; m++) {
      var span = document.createElement('span');
      span.className = 'ms-item';
      span.textContent = currentItem.emoji;
      box.appendChild(span);
    }
    msScatter.appendChild(box);
  }
