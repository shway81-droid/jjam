/* games/decimal-build/game.js */

(function () {
  'use strict';

  // ─── 소수 협력 ─────────────────────────────────────────────────────────
  // 목표 소수(예: 3.4)가 주어지면 P1은 일의 자리(정수 부분), P2는 소수 첫째 자리를
  // 골라 함께 만든다. 방해 카드에는 "자리를 바꾼 숫자"를 섞어 소수 자릿값 판단을 유도한다.
  ...
  // ─── 문제 한 개 빌드 ─────────────────────────────────────────────────────
  // 정답: P1=일의 자리(정수부), P2=소수 첫째 자리. 함정: 서로의 자리 숫자를 보기에 섞음.
  function buildProblem(pair) {
    var whole = pair.whole;   // 일의 자리 (1~9)
    var tenth = pair.tenth;   // 소수 첫째 자리 (1~9)
    return {
      label: whole + '.' + tenth,
      whole: whole,
      tenth: tenth,
      p1: pickFour(whole, tenth),   // 일의 자리 보기 (함정: 소수 자리 숫자)
      p2: pickFour(tenth, whole)    // 소수 자리 보기 (함정: 일의 자리 숫자)
    };
  }

  // ─── 이번 게임의 목표 소수 5개 (서로 다름, 두 자리가 서로 다른 수) ──────────
  function buildTargets() {
    ...
    while (picked.length < TOTAL_ROUNDS && guard < 500) {
      ...
      var tenth = randInt(1, 9);
      // 자릿값 판단이 의미 있도록 정수부·소수부가 같은 수(3.3 등)는 피함
      if (whole === tenth) continue;
      var key = whole + '.' + tenth;
      if (seen[key]) continue;
      seen[key] = true;
      picked.push({ whole: whole, tenth: tenth });
    }
    return picked;
  }
  ...
  function nextRound() {
    ...
    var problem = problems[currentRound];
    targetVal.textContent = problem.label;
