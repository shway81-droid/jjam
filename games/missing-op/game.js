/* games/missing-op/game.js — 패턴 A (4지선다 동시반응) — 알맞은 연산 기호 */
'use strict';
...
// ── Problem Data: 빈칸에 알맞은 연산 기호 (정답 유일) ────────
const OP_SYMS = ['+', '-', '×', '÷'];
function buildProblems() {
  const OPS = [
    { sym: '+', f: (a, b) => a + b, ok: () => true },
    { sym: '-', f: (a, b) => a - b, ok: (a, b) => a > b },
    { sym: '×', f: (a, b) => a * b, ok: () => true },
    { sym: '÷', f: (a, b) => a / b, ok: (a, b) => b !== 0 && a % b === 0 },
  ];
  const list = [];
  for (let a = 2; a <= 9; a++) {
    for (let b = 2; b <= 9; b++) {
      OPS.forEach(op => {
        if (!op.ok(a, b)) return;
        const r = op.f(a, b);
        if (r < 0 || !Number.isInteger(r)) return;
        // 유일성: 같은 결과를 내는 다른 기호가 있으면 제외 (정답 모호성 방지)
        const clash = OPS.some(o => o.sym !== op.sym && o.ok(a, b) && o.f(a, b) === r);
        if (clash) return;
        list.push({ word: `${a} ○ ${b} = ${r}`, opposite: op.sym });
      });
    }
  }
  return list;
...
// 4지선다: 네 가지 연산 기호 (+, -, ×, ÷) — 정답은 하나뿐
function makeChoices(currentPair) {
  return shuffle(OP_SYMS.slice());
}
