...
const HANDS = {
  ...
  scissors: { name: '가위', svg: handScissors, beats: 'paper' },
};

const HAND_KEYS = ['rock', 'paper', 'scissors'];

// 어떤 손이 입력 손을 이기는가?
function keyThatBeats(handKey) {
  return HAND_KEYS.find(k => HANDS[k].beats === handKey);
}

// "이기는 손" / "지는 손" / "비기는 손"
const PROMPTS = [
  { id: 'win',  text: '이기는 손은?',  pick: (k) => keyThatBeats(k) },
  { id: 'lose', text: '지는 손은?',   pick: (k) => HANDS[k].beats   },
  ...
  for (let rep = 0; rep < 4; rep++) {
    HAND_KEYS.forEach((handKey) => {
      PROMPTS.forEach((prompt) => {
        pool.push({ handKey, promptId: prompt.id });
      });
    });
  }
  return pool;
...
let scores         = [];
let roundLog       = [];
let currentRound   = null;   // { opponentKey, promptId, correctKey }
let currentChoices = HAND_KEYS.slice();
let dqSet          = new Set();
let phase          = 'idle';
...
const introHandRow     = document.getElementById('introHandRow');
...
function renderIntroHands() {
  ...
  HAND_KEYS.forEach(k => {
    ...
    wrap.className = 'intro-hand-thumb';
    wrap.innerHTML = HANDS[k].svg();
    introHandRow.appendChild(wrap);
  });
}
renderIntroHands();
...
function populateAnswerBtns() {
  for (let i = 0; i < playerCount; i++) {
    ...
    currentChoices.forEach((handKey) => {
...
function handleAnswerTap(playerIdx, chosenKey, btn) {
  ...
  const correct = (chosenKey === currentRound.correctKey);
...
function resolveRound(winnerIdx) {
  ...
  getAnswerBtns(winnerIdx).forEach(btn => {
    if (btn.dataset.choice === currentRound.correctKey) {
  ...
  roundLog.push({
    summary: `${HANDS[currentRound.opponentKey].name} → ${HANDS[currentRound.correctKey].name}`,
    winnerIdx,
    dqPlayers: [...dqSet],
    timedOut: false,
  });
...
function handleTimeout() {
  ...
  problemStatus.textContent = `시간 초과! 정답: ${HANDS[currentRound.correctKey].name}`;
...
function loadRound() {
  ...
  const prompt = getPromptDef(r.promptId);
  const correctKey = prompt.pick(r.handKey);
  currentRound = {
    opponentKey: r.handKey,
    promptId: r.promptId,
    correctKey: correctKey,
  };
  currentChoices = shuffle(HAND_KEYS);
  dqSet = new Set();

  questionCounter.textContent = `${roundIdx + 1} / ${TOTAL_ROUNDS}`;
  handDisplay.innerHTML = HANDS[r.handKey].svg();
  problemPrompt.textContent = prompt.text;
...
function showResult() {
  ...
  roundLog.forEach((log, idx) => {
    ...
    let cells = `<td style="text-align:left;font-size:0.82rem;">${idx + 1}. ${log.summary}</td>`;
