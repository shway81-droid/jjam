/* games/digit-sum/game.js — 패턴 A (4지선다 동시반응) — 자릿수의 합 */
'use strict';
...
// ── Problem Data: 각 자리 숫자의 합 ─────────────────────────
function buildProblems() {
  const nums = [
    23, 45, 52, 74, 68, 91, 37, 86, 19, 50,
    128, 345, 406, 517, 263, 780, 634, 159, 472, 905,
    88, 76, 64, 29, 13, 99, 203, 321, 444, 555, 618, 730,
  ];
  return nums.map(n => {
    const s = String(n).split('').reduce((a, d) => a + Number(d), 0);
    return { word: String(n), opposite: String(s), a: s };
  });
}
...
// 4지선다: 정답(자릿수 합) + 가까운 오답 3개
function makeChoices(currentPair) {
  const c = currentPair.a;
  const pool = [];
  const push = v => { if (v > 0 && v !== c && !pool.includes(v)) pool.push(v); };
  [1, -1, 2, -2, 3, -3, 4].forEach(d => push(c + d));
  const wrong = shuffle(pool).slice(0, 3).map(String);
  return shuffle([String(c), ...wrong]);
}
