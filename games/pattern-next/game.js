/* games/pattern-next/game.js — 패턴 A (4지선다 동시반응) — 다음 무늬 찾기 */
'use strict';
...
// ── Pattern Data: 32 repeating-pattern problems ──────────────
// seq = 화면에 보이는 무늬 줄, answer = 다음에 와야 할 무늬
const ALL_PATTERNS = [
  // AB 반복
  { seq: ['🔴','🔵','🔴','🔵','🔴'], answer: '🔵' },
  { seq: ['🟡','🟢','🟡','🟢','🟡'], answer: '🟢' },
...
// 4지선다: 정답(다음 무늬) + 팔레트에서 다른 3개 무늬 (정답 제외, 중복 없음)
function makeChoices(item) {
  const correct = item.answer;
  const pool = shuffle(PALETTE.filter(p => p !== correct)).slice(0, 3);
...
function handleAnswerTap(playerIdx, chosenName, btn) {
  ...
  const correct = (chosenName === currentItem.answer);
...
function resolveRound(winnerIdx) {
  ...
  getAnswerBtns(winnerIdx).forEach(btn => {
    if (btn.dataset.choice === currentItem.answer) {
...
function loadRound() {
  ...
  wordDisplay.textContent = currentItem.seq.join(' ') + ' ❔';
...
// ── Start game ───────────────────────────────────────────────
function startGame() {
  gameRounds  = shuffle(ALL_PATTERNS).slice(0, TOTAL_ROUNDS);
  roundIdx    = 0;
...
function showResult() {
  ...
  roundLog.forEach((log, idx) => {
    ...
    let cells = `<td style="text-align:left;font-size:0.82rem;">${idx + 1}. ${log.seq} → ${log.answer}</td>`;
