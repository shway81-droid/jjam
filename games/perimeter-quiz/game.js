...
// ── Question generation (둘레 = 도형의 모든 변의 길이의 합) ──────
// 매 라운드 도형을 무작위로 생성. { q, a, o:[오답 3개] } 형태로 반환하면
// 아래 buildQuestion(item)이 그대로 4지선다로 만든다.
function makeWrongs(peri) {
  // 정답 주변 값 + 흔한 실수값(가로+세로만 더하기 등)에서 서로 다른 오답 3개
  const cand = [peri + 1, peri - 1, peri + 2, peri - 2, peri + 3, peri - 3,
                peri + 4, peri - 4, peri + 5, peri - 5];
  const pool = shuffle(cand);
  const wrongSet = new Set();
  for (let i = 0; i < pool.length && wrongSet.size < 3; i++) {
    const w = pool[i];
    if (w > 0 && w !== peri) wrongSet.add(w);
  }
  return Array.from(wrongSet).map(String);
}

function generateQuestion() {
  const type = rand(0, 3); // 0 정사각형 · 1 직사각형 · 2 정삼각형 · 3 삼각형
  let q, peri;

  if (type === 0) {
    const s = rand(2, 12);
    peri = 4 * s;
    q = `한 변의 길이가 ${s}cm인 정사각형의 둘레는 몇 cm일까요?`;
  } else if (type === 1) {
    let w = rand(2, 12), h = rand(2, 12);
    if (w === h) h = (h % 12) + 1;
    peri = 2 * (w + h);
    q = `가로 ${w}cm, 세로 ${h}cm인 직사각형의 둘레는 몇 cm일까요?`;
  } else if (type === 2) {
    const s = rand(2, 12);
    peri = 3 * s;
    q = `한 변의 길이가 ${s}cm인 정삼각형의 둘레는 몇 cm일까요?`;
  } else {
    const a = rand(2, 10), b = rand(2, 10), c = rand(2, 10);
    peri = a + b + c;
    q = `세 변의 길이가 각각 ${a}cm, ${b}cm, ${c}cm인 삼각형의 둘레는 몇 cm일까요?`;
  }

  return { q, a: String(peri), o: makeWrongs(peri) };
}
...
// ── Question flow ─────────────────────────────────────────────
function startGame() {
  gameQuestions = Array.from({ length: TOTAL_QUESTIONS }, () => generateQuestion());
